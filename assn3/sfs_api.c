...
dir_entry_t root_dir[MAX_FILES];                // directory cache - we keep ALL directory blocks in memory
inode_t inode_table[MAX_FILES];                 // inode cache
fd_entry_t open_fd_table[MAX_OPEN_FILES];       // open file descriptor cache
char free_blocks[DATA_BLOCKS];                  // includes directory
...
int allocate_empty_blocks(int inode_idx, int num_blocks_needed) {
    ...
    int start = 0;
    int p = 0;
    //int tot = start_blocks + num_blocks_needed;
    for (j=0; j<p; j++) {
    //while (curr_blocks < (start_blocks+num_blocks_needed)) {
    ...
    if (ind_ptr_block == 1)
        inode_table[inode_idx].indirect_ptr = ptrs[num];
...
void mksfs(int fresh)
{
	...
	if (fresh) {                                            // initialise fresh disk
        ...
        bzero(&open_fd_table, sizeof(fd_entry_t) * MAX_FILES);
        ...
        read_blocks(DATA_BITMAP_ADDR, DATA_BITMAP_SIZE, buf);
...
int sfs_getnextfilename(char *fname)
{
    ...
    if (idx_in_dir < num_files) {
        ...
        idx_in_dir++;
        open_fd_table[ROOT_NUM].rw_ptr += sizeof(dir_entry_t);
        return (idx_in_dir-1);
    }
